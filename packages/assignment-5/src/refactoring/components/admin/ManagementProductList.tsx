...
import { EditProductForm, DiscountProductForm } from './';
interface Props {
  products: Product[];
...
export const ManagementProductList = ({ products, onProductUpdate }: Props) => {
  ...
  //*Hooks
  const { edit, editingProduct, editProperty, submit: editSubmit } = useEditProduct(products, onProductUpdate);
      ...
      {products.map((product, index) => (
        <div key={product.id} data-testid={`product-${index + 1}`} className="bg-white p-4 rounded shadow">
          <button
            ...
            onClick={() => {
              toggleProductEditForm(product.id);
            }}
          ...
          {openProductIds.has(product.id) && (
            <div className="mt-2">
              {editingProduct && editingProduct.id === product.id ? (
                ...
                >
                  <DiscountProductForm products={products} productId={product.id} onProductUpdate={onProductUpdate} />
                </EditProductForm>
              ) : (
                <div>
